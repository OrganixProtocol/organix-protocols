...
// Used to have strongly-typed access to internal mutative functions in Organix
interface IOrganixInternal {
...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[3] = CONTRACT_ORGANIX;
        ...
        )
    {
        ...
        for (uint i = 0; i < numEntries; i++) {
            ...
            // OIP-65 settlements where the amount at end of waiting period is beyond the threshold, then
    ...
    function exchange(
        ...
        bytes32 trackingCode
    ) external onlyOrganixorSynth returns (uint amountReceived, IVirtualSynth vSynth) {
        uint fee;
        ...
        if (trackingCode != bytes32(0)) {
            IOrganixInternal(address(organix())).emitExchangeTracking(
                trackingCode,
                destinationCurrencyKey,
                amountReceived,
                fee
    ...
    function _updateOGXIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal {
        bool includesOUSD = currencyKeys[0] == oUSD || currencyKeys[1] == oUSD;
        uint numKeys = includesOUSD ? 2 : 3;
        ...
        if (!includesOUSD) {
            keys[2] = oUSD; // And we'll also update oUSD to account for any fees if it wasn't one of the exchanged currencies
        ...
        )
    {
        ...
        if (fee > 0) {
            ...
            fee = exchangeRates().effectiveValue(destinationCurrencyKey, fee, oUSD);

            // Remit the fee in oUSDs
            issuer().synths(oUSD).issue(feePool().FEE_ADDRESS(), fee);

            // Tell the fee pool about this
            feePool().recordFeePaid(fee);
        }

        // Note: As of this point, `fee` is denominated in oUSD.

        // Nothing changes as far as issuance data goes because the total value in the system hasn't changed.
        // But we will update the debt snapshot in case exchange rates have fluctuated since the last exchange
        // in these currencies
        _updateOGXIssuedDebtOnExchange([sourceCurrencyKey, destinationCurrencyKey], [sourceRate, destinationRate]);

        // Let the DApps know there was a Synth exchange
        IOrganixInternal(address(organix())).emitSynthExchange(
            from,
            sourceCurrencyKey,
            sourceAmountAfterSettlement,
            destinationCurrencyKey,
    ...
    function reclaim(
        ...
        IOrganixInternal(address(organix())).emitExchangeReclaim(from, currencyKey, amount);
    }
    ...
    function refund(
        ...
        IOrganixInternal(address(organix())).emitExchangeRebate(from, currencyKey, amount);
    }
        ...
        returns (uint exchangeFeeRate)
    {
        ...
        if (sourceCurrencyKey == oUSD || destinationCurrencyKey == oUSD) {
            return exchangeFeeRate;
        }

        // Is this a swing trade? long to short or short to long skipping oUSD.
        if (
            (sourceCurrencyKey[0] == 0x6F && destinationCurrencyKey[0] == 0x69) ||
            (sourceCurrencyKey[0] == 0x69 && destinationCurrencyKey[0] == 0x6F)
        ) {
    ...
    modifier onlyOrganixorSynth() {
        IOrganix _organix = organix();
        require(
            msg.sender == address(_organix) || _organix.synthsByAddress(msg.sender) != bytes32(0),
            "Exchanger: Only organix or a synth contract can perform this action"
        );
        _;
    }

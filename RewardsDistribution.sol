...
contract RewardsDistribution is Owned, IRewardsDistribution {
    ...
    /**
     * @notice Address of the Organix ProxyERC20
     */
    address public organixProxy;
    ...
    /**
     * @dev _authority maybe the underlying organix contract.
     * Remember to set the authority on a organix upgrade
     */
    constructor(
        address _owner,
        address _authority,
        address _organixProxy,
        address _rewardEscrow,
        address _feePoolProxy
    ) public Owned(_owner) {
        authority = _authority;
        organixProxy = _organixProxy;
    ...
    function setOrganixProxy(address _organixProxy) external onlyOwner {
        organixProxy = _organixProxy;
    }
    ...
    function distributeRewards(uint amount) external returns (bool) {
        ...
        require(organixProxy != address(0), "OrganixProxy is not set");
        require(feePoolProxy != address(0), "FeePoolProxy is not set");
        require(
            IERC20(organixProxy).balanceOf(address(this)) >= amount,
        ...
        for (uint i = 0; i < distributions.length; i++) {
            if (distributions[i].destination != address(0) || distributions[i].amount != 0) {
                ...
                // Transfer the OGX
                IERC20(organixProxy).transfer(distributions[i].destination, distributions[i].amount);

                // If the contract implements RewardsDistributionRecipient.sol, inform it how many OGX its received.
        ...
        IERC20(organixProxy).transfer(rewardEscrow, remainder);

...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[4] = CONTRACT_SYNTHOUSD;
    ...
    function _synthoUSD() internal view returns (ISynth) {
    ...
    function setNewManager() external onlyOwner {
       address newManager = getNewCollateralManager(address(this));
       require(newManager != address(0), "CollateralManager address is invalid " );
       manager = ICollateralManager(newManager);
       emit CollateralManagerUpdated(newManager);
    }
    ...
    function _open(
        ...
        // 4. Check we haven't hit the debt cap for non OGX collateral.
        ...
        // 13. If its short, convert back to oUSD, otherwise issue the loan.
        if (short) {
            _synthoUSD().issue(msg.sender, _exchangeRates().effectiveValue(currency, loanAmountMinusFee, oUSD));
    ...
    function _repayWithCollateral(
        ...
        // 4. Get the expected amount for the exchange from borrowed synth -> oUSD.
        (uint expectedAmount, uint fee, ) = _exchanger().getAmountsForExchange(payment, loan.currency, oUSD);
    ...
    function _draw(uint id, uint amount) internal rateIsValid issuanceIsActive returns (uint, uint) {
        ...
        if (loan.short) {
            ...
            _synthoUSD().issue(msg.sender, _exchangeRates().effectiveValue(loan.currency, amountMinusFee, oUSD));
    ...
    // Take an amount of fees in a certain synth and convert it to oUSD before paying the fee pool.
    function _payFees(uint amount, bytes32 synth) internal {
        if (amount > 0) {
            if (synth != oUSD) {
                amount = _exchangeRates().effectiveValue(synth, amount, oUSD);
            }
            _synthoUSD().issue(_feePool().FEE_ADDRESS(), amount);

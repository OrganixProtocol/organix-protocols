...
contract FeePoolState is Owned, LimitedSetup {
     ...
     * @param debtEntryIndex The index in the global debt ledger. organix.organixState().issuanceData(account)
     * @param currentPeriodStartDebtIndex The startingDebtIndex of the current fee period
     * @dev onlyFeePool to call me on organix.issue() & organix.burn() calls to store the locked OGX
    ...
    /**
     * @notice Import issuer data from organixState.issuerData on FeePeriodClose() block #

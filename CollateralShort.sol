...
contract CollateralShort is Collateral {
    ...
    function open(
        ...
        IERC20(address(_synthoUSD())).transferFrom(msg.sender, address(this), collateral);
    ...
    function close(uint id) external returns (uint amount, uint collateral) {
        ...
        IERC20(address(_synthoUSD())).transfer(msg.sender, collateral);
    }
    ...
    function deposit(
        ...
        require(amount <= IERC20(address(_synthoUSD())).allowance(msg.sender, address(this)), "Allowance too low");

        IERC20(address(_synthoUSD())).transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint id, uint amount) external returns (uint principal, uint collateral) {
        ...
        IERC20(address(_synthoUSD())).transfer(msg.sender, amount);
    }
    ...
    function liquidate(
        ...
        IERC20(address(_synthoUSD())).transfer(msg.sender, collateralLiquidated);
    }
}

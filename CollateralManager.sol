...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    // The maximum amount of debt in oUSD that can be issued by non OGX collateral.
    uint public maxDebt;
    ...
    function totalLong() public view returns (uint ousdValue, bool anyRateIsInvalid) {
    ...
    function totalShort() public view returns (uint ousdValue, bool anyRateIsInvalid) {
    ...
    function totalLongAndShort() public view returns (uint ousdValue, bool anyRateIsInvalid) {
        ...
        if (currencyKeys.length > 0) {
            ...
            for (uint i = 0; i < rates.length; i++) {
                ...
                ousdValue = ousdValue.add(longAmount).add(shortAmount);
                if (invalid) {
                    anyRateIsInvalid = true;
                }
            }
    ...
    function getBorrowRate() public view returns (uint borrowRate, bool anyRateIsInvalid) {
        // get the OGX backed debt.
        uint ogxDebt = _issuer().totalIssuedSynths(oUSD, true);

        // now get the non OGX backed debt.
        (uint nonOgxDebt, bool ratesInvalid) = totalLong();

        // the total.
        uint totalDebt = ogxDebt.add(nonOgxDebt);

        // now work out the utilisation ratio, and divide through to get a per second value.
        uint utilisation = nonOgxDebt.divideDecimal(totalDebt).divideDecimal(SECONDS_IN_A_YEAR);
    ...
    function exceedsDebtLimit(uint amount, bytes32 currency) external view returns (bool canIssue, bool anyRateIsInvalid) {
        uint usdAmount = _exchangeRates().effectiveValue(currency, amount, oUSD);

...
contract BaseOrganix is IERC20, ExternStateToken, MixinResolver, IOrganix {
    // ========== STATE VARIABLES ==========

    // Available Synths which can be used with the system
    string public constant TOKEN_NAME = "Organix Network Token";
    string public constant TOKEN_SYMBOL = "OGX";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[0] = CONTRACT_ORGANIXSTATE;
    ...
    function anySynthOrOGXRateIsInvalid() external view returns (bool anyRateInvalid) {
        return issuer().anySynthOrOGXRateIsInvalid();
    }
    ...
    function transferableOrganix(address account) external view returns (uint transferable) {
        (transferable, ) = issuer().transferableOrganixAndAnyRateIsInvalid(account, tokenState.balanceOf(account));
    }

    function _canTransfer(address account, uint value) internal view returns (bool) {
        (uint initialDebtOwnership, ) = organixState().issuanceData(account);

        if (initialDebtOwnership > 0) {
            (uint transferable, bool anyRateIsInvalid) =
                issuer().transferableOrganixAndAnyRateIsInvalid(account, tokenState.balanceOf(account));
            require(value <= transferable, "Cannot transfer staked or escrowed OGX");

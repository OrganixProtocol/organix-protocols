...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    uint public constant MAX_ACCUMULATION_RATIO = 1e18;

    address public AccumulationAddress;

    uint public AccumulationRate;

    uint public AccumulationAmount;
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[1] = CONTRACT_ORGANIX;
        ...
        newAddresses[6] = CONTRACT_ORGANIXSTATE;
        ...
        newAddresses[11] = CONTRACT_BNB_WRAPPER;
     ...
     * @param debtEntryIndex The index in the global debt ledger. organixState.issuanceData(account)
     * @dev onlyIssuer to call me on organix.issue() & organix.burn() calls to store the locked OGX
    ...
    function appendAccountIssuanceRecord(
        ...
        uint debtRatio,
        uint debtEntryIndex
    ) external onlyIssuerAndOrganixState {
     ...
     * @param amount oUSD amount in fees being paid.
     */
    ...
    /**
     * @notice The RewardsDistribution contract informs us how many OGX rewards are sent to RewardEscrow to be claimed.
     */
    function setRewardsToDistribute(uint amount) external {
        ...
        // Add the amount of OGX rewards to distribute on top of any rolling unclaimed amount
    ...
    function closeCurrentFeePeriod() external issuanceActive {
        ...
        _recentFeePeriodsStorage(0).startingDebtIndex = uint64(organixState().debtLedgerLength());
        _recentFeePeriodsStorage(0).startTime = uint64(now);
    ...
    function importFeePeriod(
        ...
        require(startingDebtIndex <= organixState().debtLedgerLength(), "Cannot import bad data");
     ...
     * @param ogxAmount The amount of OGX tokens.
     */
    function _recordRewardPayment(uint ogxAmount) internal returns (uint) {
        // Don't assign to the parameter
        uint remainingToAllocate = ogxAmount;
     ...
     * @param account The address to send the fees to.
     * @param ogxAmount The amount of OGX.
     */
    function _payRewards(address account, uint ogxAmount) internal notFeeAddress(account) {

        // Record vesting entry for claiming address and amount
        // OGX already minted to rewardEscrow balance
        rewardEscrow().appendVestingEntry(account, ogxAmount);
    }
    ...
    /**
     * @notice The total OGX rewards available in the system to be withdrawn
     */
     ...
     * @dev Returns two amounts, one for fees and one for OGX rewards
     */
    function feesAvailable(address account) public view returns (uint, uint) {
        ...
        // Return totalRewards as is in OGX amount
    ...
    function _effectiveDebtRatioForPeriod(
        ...
        // This is a high precision integer.
        IOrganixState _organixState = organixState();
        uint feePeriodDebtOwnership =
            _organixState
                .debtLedger(closingDebtIndex)
                .divideDecimalRoundPrecise(_organixState.debtLedger(debtEntryIndex))
    ...
    function setAccumulation( address _AccumulationAddress, uint _AccumulationRate) external optionalProxy_onlyOwner {
        require(_AccumulationRate <= MAX_ACCUMULATION_RATIO, "New Accumulation ratio cannot exceed MAX_ACCUMULATION_RATIO");
        AccumulationAddress = _AccumulationAddress;
        AccumulationRate = _AccumulationRate;
    }

    function withDrawAccumulation() external optionalProxy_onlyOwner returns (bool) {
        require(AccumulationAmount > 0, "There is no AccumulationAmount ");

        _payFees(AccumulationAddress, AccumulationAmount);
        AccumulationAmount = 0;
        emitWithDrawAccumulation(AccumulationAddress, AccumulationAmount);
        return true;
    }
    ...
    modifier onlyInternalContracts {
        ...
        require(isExchanger || isSynth || isCollateral || isBNBWrapper, "Only Internal Contracts");
        _;
    }

    modifier onlyIssuerAndOrganixState {
        bool isIssuer = msg.sender == address(issuer());
        bool isOrganixState = msg.sender == address(organixState());
        require(isIssuer || isOrganixState, "Issuer and OrganixState only");
        _;
    }
    ...
    event FeesClaimed(address account, uint oUSDAmount, uint ogxRewards);
    ...
    function emitFeesClaimed(
        ...
        proxy._emit(abi.encode(account, oUSDAmount, ogxRewards), 1, FEESCLAIMED_SIG, 0, 0, 0);
    }

    event withDrawAccumulations( address account, uint Amount);
    bytes32 private constant WITHDRAWACCUMULATION_SIG = keccak256("withDrawAccumulations(address, uint256)");

    function emitWithDrawAccumulation(
        address account,
        uint Amount
    ) internal {
        proxy._emit(abi.encode(account, Amount), 1, WITHDRAWACCUMULATION_SIG, 0, 0, 0);
    }
}

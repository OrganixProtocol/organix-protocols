...
contract CollateralUtil is ICollateralUtil, ICollateralLoan, MixinSystemSettings {
    ...
    function getCollateralRatio(Loan calldata loan, bytes32 collateralKey) external view returns (uint cratio) {
        uint cvalue = _exchangeRates().effectiveValue(collateralKey, loan.collateral, oUSD);
        uint dvalue = _exchangeRates().effectiveValue(loan.currency, loan.amount.add(loan.accruedInterest), oUSD);
     ...
     * D = debt value in oUSD
     * V = collateral value in oUSD
    ...
    function liquidationAmount(
        ...
        uint debtValue = _exchangeRates().effectiveValue(loan.currency, loan.amount.add(loan.accruedInterest), oUSD);
        uint collateralValue = _exchangeRates().effectiveValue(collateralKey, loan.collateral, oUSD);
        ...
        uint oUSDamount = dividend.divideDecimal(divisor);

        return _exchangeRates().effectiveValue(oUSD, oUSDamount, loan.currency);
    }

...
contract Liquidations is Owned, MixinSystemSettings, ILiquidations {
    ...
    // refactor to organix storage eternal storage contract once that's ready
    ...
    function isOpenForLiquidation(address account) external view returns (bool) {
        uint accountCollateralisationRatio = organix().collateralisationRatio(account);

        // Liquidation closed if collateral ratio less than or equal target issuance Ratio
        // Account with no OGX collateral will also not be open for liquidation (ratio is 0)
    ...
    // check OGX rate is not stale
    function flagAccountForLiquidation(address account) external rateNotInvalid("OGX") {
        ...
        uint accountsCollateralisationRatio = organix().collateralisationRatio(account);
    ...
    // Check OGX rate is not stale
    function checkAndRemoveAccountInLiquidation(address account) external rateNotInvalid("OGX") {
        systemStatus().requireSystemActive();
